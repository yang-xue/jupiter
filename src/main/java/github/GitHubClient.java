...
public class GitHubClient {
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(entity.getContent())); // yiduan yiduan du
																									// meiduan 8kb
	...
	private List<Item> getItemList(JSONArray array) {
		...
		List<String> descriptionList = new ArrayList<>(); // put all jobs' description together, use MonkeyLearn do
															// batch processing
		...
		if (descriptionList.size() > 0) {
			String[] descriptionArray = descriptionList.toArray(new String[descriptionList.size()]); // Convert list to
																										// an array of
																										// the same
																										// type.
			...
			for (int i = 0; i < keywords.size(); ++i) {
				...
				// Why do we use HashSet but List here? Avoid duplicated keywords
